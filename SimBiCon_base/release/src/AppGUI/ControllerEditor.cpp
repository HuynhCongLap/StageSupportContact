	...
	if (shadowMode == false){
		...
		if (Globals::drawContactForces){
			...
			for (uint i=0;i<cfs->size();i++){
				...
				GLUtils::drawCone(0.03, c->f * 0.0008, c->cp+c->f*0.0008);
			}

			Vector3d forceOnLeftFoot = con->getForceOnFoot(con->getRBBySymbolicName("lFoot"), cfs);
			Vector3d forceOnRightFoot = con->getForceOnFoot(con->getRBBySymbolicName("rFoot"), cfs);
